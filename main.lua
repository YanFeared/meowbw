...
local function downloadFile(path, func)
	...
	if not isfile(path) then
		local suc, res = pcall(function()
			return game:HttpGet('https://raw.githubusercontent.com/YanFeared/meowbw/'..readfile('catrewrite/profiles/commit.txt')..'/'..select(1, path:gsub('catrewrite/', '')), true)
		end)
		...
		if (not teleportedServers) and (not shared.VapeIndependent) and vape.AutoTeleport.Enabled then
			...
			local teleportScript = [[
				...
				if shared.VapeDeveloper then
					...
					loadstring(game:HttpGet('https://raw.githubusercontent.com/YanFeared/meowbw/'..readfile('catrewrite/profiles/commit.txt')..'/loader.lua', true), 'loader')(sharedData)
				end
			]]
			if shared.VapeDeveloper then
	...
	if isfile('catrewrite/games/'..game.PlaceId..'.lua') then
		...
		if not shared.VapeDeveloper then
			local success, result = pcall(function()
				return game:HttpGet('https://raw.githubusercontent.com/YanFeared/meowbw/'..readfile('catrewrite/profiles/commit.txt')..'/games/'..game.PlaceId..'.lua')
			end)
