...
local _, subbed = pcall(function()
    return game:HttpGet('https://github.com/YanFeared/meowbw')
end)
    ...
    local function downloadFile(path, func)
        if not isfile(path) or (not isfile('catrewrite/profiles/commit.txt') or readfile('catrewrite/profiles/commit.txt') ~= commit) and not shared.VapeDeveloper then
            local suc, res = pcall(function()
                return game:HttpGet('https://raw.githubusercontent.com/YanFeared/meowbw/'.. commit.. '/' ..select(1, path:gsub('catrewrite/', '')), true)
            end)
